...
# Validates that releasing the same snapshot twice filters already-released components
CLEANUP="true"
...
get_filtered_count_from_result() {
    local release_name=$1

    local skip_release
    skip_release=$(get_taskrun_result "${release_name}" "filter-already-released-images" "skip_release")
    
    local push_snapshot_skipped
    if is_taskrun_skipped "${release_name}" "push-snapshot"; then
        push_snapshot_skipped="true"
    else
        push_snapshot_skipped="false"
    fi

    # Return 1 if all components were filtered, 0 otherwise
    if [ "${skip_release}" = "true" ] && [ "${push_snapshot_skipped}" = "true" ]; then
        echo "1"
    else
        echo "0"
    fi
...
FILTERED_COUNT_1=$(get_filtered_count_from_result "${RELEASE_1_NAME}")
...
echo "Getting filter task results..."
FILTERED_COUNT_2=$(get_filtered_count_from_result "${RELEASE_2_NAME}")
...
# Exit to prevent run-test.sh from executing its standard flow
exit 0
