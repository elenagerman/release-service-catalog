...
200 pre-built multi-arch images** (`resources/static-image-pool-stable.txt`).
There is no image build step — images already exist in Quay and are selected
directly from the pool.

| Property | Value |
|---|---|
| Components (default) | 200 (`COMPONENT_COUNT`) |
| Architectures per image | 4 (amd64, arm64, s390x, ppc64le) |
| Total digests (full run) | ~800 |
| Tags per component | 4 (2 run-unique + 2 static; ×200 = 800 total, 400 fresh per run) |
| Managed pipeline duration | ~2 h 14 min – ~2 h 42 min (avg ~2 h 26 min) |
| Dominant variable task | `rh-sign-image` (42m–59m; retried in all observed runs) |
...
> `rh-sign-image` has been observed to stall ~34 min on its first attempt before
> the retry completes — this is the main source of variability between runs.

`COMPONENT_COUNT` controls how many images are selected from the pool (0 = all 200).
...
- `COMPONENT_COUNT` — number of images to select from the pool (default: 200;
  0 = all)
