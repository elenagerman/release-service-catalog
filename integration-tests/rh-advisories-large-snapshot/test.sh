...
#
# Stress test for the rh-advisories pipeline using a static pool of pre-built multi-arch images.
...
# Per-component tag suffix injected into RPA mapping by patch_rpa_with_snapshot_components().
# Each component receives tags of the form <component_name>-<suffix> so no two components
# ever push to the same (repository, tag) pair.
# Override with an explicit value (e.g. RELEASE_MAPPING_TAG=myrun ./test.sh).
# Default: the suite-wide uuid (PipelineRun UID prefix or random hex for local runs).
RELEASE_MAPPING_TAG="${RELEASE_MAPPING_TAG:-}"

# Signing configuration — leave empty to use the value already in the RPA template.
# Set explicitly to override (e.g. SIGNING_CONFIGMAP_NAME=hacbs-signing-pipeline-config-staging-e2e).
SIGNING_CONFIGMAP_NAME="${SIGNING_CONFIGMAP_NAME:-}"
...
if [ -n "${RELEASE_MAPPING_TAG}" ]; then
    echo "  Per-component tag suffix: ${RELEASE_MAPPING_TAG} (explicit override)"
else
    echo "  Per-component tag suffix: ${uuid} (from PipelineRun UID / uuid)"
fi
echo "  Signing ConfigMap: ${SIGNING_CONFIGMAP_NAME:-(from RPA template)}"
echo ""
...
# Ensure CONSOLE_URL has trailing slash for URL construction (skip if empty — detection failed)
[[ -n "${CONSOLE_URL}" && "${CONSOLE_URL}" != */ ]] && CONSOLE_URL="${CONSOLE_URL}/"
...
patch_rpa_with_snapshot_components() {
    ...
    local _run_suffix="${RELEASE_MAPPING_TAG:-${uuid:-$(date +%Y%m%d-%H%M%S)}}"
    ...
    # sign.configMapName is only patched when SIGNING_CONFIGMAP_NAME is explicitly set.
    local patch_json
    if [ -n "${SIGNING_CONFIGMAP_NAME}" ]; then
        patch_json=$(jq -n \
...
create_kubernetes_resources() {
    ...
    # The embargo-check task hardcodes secretName: konflux-advisory-jira-secret in its
    # volume spec and ignores the RPA's jiraAdvisorySecret field for pod mounting.
    # This permanent cluster secret must exist in the managed namespace before the
    # managed pipeline runs.  Create/update it from our per-run advisory jira secret
    # so the test is self-contained and does not depend on manual cluster setup.
    # We use 'apply' (not 'create') so concurrent runs safely overwrite each other
    # with the same token, and the secret survives across test runs.
    echo "  Ensuring konflux-advisory-jira-secret in ${managed_namespace} ..." >&2
    local _per_run_jira_secret="advisory-jira-secret-${component_name}"
    if kubectl get secret "${_per_run_jira_secret}" -n "${managed_namespace}" \
            --ignore-not-found 2>/dev/null | grep -q "${_per_run_jira_secret}"; then
        kubectl get secret "${_per_run_jira_secret}" \
            -n "${managed_namespace}" -o json 2>/dev/null \
            | jq 'del(.metadata.resourceVersion, .metadata.uid,
                      .metadata.creationTimestamp, .metadata.ownerReferences,
                      .metadata.labels, .metadata.annotations)
                  | .metadata.name = "konflux-advisory-jira-secret"' \
            | kubectl apply -f - -n "${managed_namespace}" >/dev/null 2>&1 \
            && echo "  ✅ konflux-advisory-jira-secret ensured" >&2 \
            || echo "  ⚠️  Could not ensure konflux-advisory-jira-secret (non-fatal)" >&2
    else
        echo "  ⚠️  Per-run jira secret ${_per_run_jira_secret} not found yet — skipping" >&2
    fi
    ...
    # Patch RPA with actual component names from snapshot
