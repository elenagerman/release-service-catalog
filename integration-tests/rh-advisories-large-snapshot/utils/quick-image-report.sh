...
# Check both spec.containerImage (recovered) and status.lastPromotedImage (built)
...
while IFS='|' read -r NAME VERSION SPEC_IMAGE STATUS_IMAGE SECRET PLATFORMS; do
    # Check image (check both spec.containerImage and status.lastPromotedImage)
    # Prefer status (for built components), fall back to spec (for recovered)
    IMAGE="${STATUS_IMAGE}"
