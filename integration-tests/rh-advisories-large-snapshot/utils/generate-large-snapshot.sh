...
# Validate COMPONENT_COUNT is a non-negative integer (0 = use all images in pool)
if ! [[ "${COMPONENT_COUNT}" =~ ^[0-9]+$ ]]; then
    echo "Error: COMPONENT_COUNT must be a non-negative integer (got: '${COMPONENT_COUNT}')" >&2
    exit 1
fi
...
# Resolve COMPONENT_COUNT=0 to "use all images in pool" (documented behaviour)
if [ "${COMPONENT_COUNT}" -eq 0 ]; then
    COMPONENT_COUNT=${POOL_SIZE}
    echo "   COMPONENT_COUNT=0 → using all ${POOL_SIZE} available images" >&2
fi
